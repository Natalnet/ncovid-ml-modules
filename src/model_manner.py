...
class LSTMRegressor:
    def __init__(
        self, n_input, n_lstm_nodes, dropout, n_timesteps, n_features, n_outputs
    ):
        ...
        self.stop_training = EarlyStopping(
            monitor="loss",
            mode="min",
            verbose=0,
    ...
    def __model_architecture(self):
        ...
        lstm_2 = LSTM(self.n_lstm_nodes, activation="relu", return_sequences=True)(
            repeat_vect
        )
        if self.dropout != 0.0 and self.dropout is not None:
            lstm_2 = Dropout(self.dropout)(lstm_2)
        time_dist_layer = TimeDistributed(
            Dense(np.floor(self.n_lstm_nodes / 2), activation="relu")
        )(lstm_2)
    ...
    def fit_model(self, data, epochs=None, batch_size=None, verbose=0):
        if epochs and batch_size:
            ...
            self.__fitting(
                data.x, data.y, epochs, configs_manner.model_batch_size, verbose
            )
        elif batch_size and not epochs:
            self.__fitting(
                data.x, data.y, configs_manner.model_train_epochs, batch_size, verbose
            )
        else:
            self.__fitting(
                data.x,
...
def build_model(model_config):
    if model_config:
        ...
        return LSTMRegressor(
            n_input, n_lstm_nodes, dropout, n_timesteps, n_features, n_outputs
        )
    return None
